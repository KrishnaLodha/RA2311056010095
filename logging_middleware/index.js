const ACCESS_TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..-GB_iMQ6kbyzWDtGBZ34uo8hMaatjq8IcFMKvxMLH6s";

/**
 * Log function that sends logs to the Affordmed log API.
 * @param {string} stack - Allowed: 'backend' | 'frontend'
 * @param {string} level - Allowed: 'debug' | 'info' | 'warn' | 'error' | 'fatal'
 * @param {string} pkg - The package name (e.g., 'handler', 'service', etc.)
 * @param {string} message - Descriptive message
 */
async function Log(stack, level, pkg, message) {
    try {
        const response = await fetch("http://20.207.122.201/evaluation-service/logs", {
            method: "POST",
            headers: {
                "Authorization": `Bearer ${ACCESS_TOKEN}`,
                "Content-Type": "application/json"
            },
            body: JSON.stringify({
                stack: stack,
                level: level,
                package: pkg,
                message: message
            })
        });
        
        // No console logging as per requirement.
        // We just wait for the request to complete.
        if (!response.ok) {
            // Optional: Handle token expiration internally or silently fail
            // We'll leave it silent to strictly adhere to "no console.log" rule.
        }
    } catch (error) {
        // Silently fail as per no console.log requirement
    }
}

module.exports = { Log };
